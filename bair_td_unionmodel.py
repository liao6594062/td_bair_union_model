...
连续变量的相关性分析
"""
init_choose_name_desc=pd.read_excel('D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归/init_choose_name_desc_20190809.xlsx')
init_choose_table=init_choose_name_desc.loc[init_choose_name_desc['是否选用'].isin(['是']),:]#手动去除一些不能用的变量
...
error_cols,df_choose3=main(df_choose2,col,'D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归/')
...
df_high_cor1.to_excel("D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归/df_high_cor.xlsx")


## 相关性去除iv更低的变量
cat_less_iv=pd.read_excel('D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归/cat_less_iv_1.xlsx')
cat_less_iv=cat_less_iv.set_index('var_name').loc[str_columns,'IV'].reset_index().rename(columns={'IV':'iv'})
df_hig_iv=pd.read_excel("D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归/high_iv_chi.xlsx")
df_hig_iv=pd.concat([df_hig_iv,cat_less_iv],axis=0)
df_hig_iv.to_csv('D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归/df_hig_iv_all.csv',index=False)
df_high_cor1=pd.read_excel("D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归/df_high_cor.xlsx")
...
main(df_choose5,remain_col,"D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归/final_test_num_bin_badrate1")
