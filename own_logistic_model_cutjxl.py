# -*- coding: utf-8 -*-
"""
Created on Thu Sep  5 10:01:29 2019
...
de_dict_var = pd.read_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/DE_Var_Select_20190808.xlsx')
...
vars_count_table=pd.read_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/de_dict_vars_20190808.xlsx') #手动增加了同盾外部评分
...
#    cat_less20 = ["deci_apt_facetrial_housetype","deci_cont02_relationship","deci_jxl_contact2_rel","deci_vehicle_minput_lastmortgagerinfo"]
    high_iv_name = v2_cat_woe_iv(df,cat_less20,target,0,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/") # 计算woe 和IV值
    group = []
    ...
    print(less_iv_name)
    less_iv_name.to_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/cat_less_iv_1.xlsx")
    df = v2_equif_bin(df, cat_over20, target)
    cols_name = [i+"_Bin" for i in cat_over20]
    over_iv_name = v2_cat_woe_iv(df, cols_name, target,0,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/")  # 计算woe 和IV值
    over_iv_name.to_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/cat_more_iv_2.xlsx")
...
#init_choose_name_desc.to_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/init_choose_name_desc_20190905.xlsx',index=False)
init_choose_name_desc=pd.read_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/init_choose_name_desc_20190905.xlsx')
    ...
    for col in num_var:
        ...
        if -99 not in set(df[col]):
            ...
            if len(cutOff)!=0:
               ...
               regroup.to_csv("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/regroup_Chi_cutbin__.csv", mode="a", header=True)
               print(regroup)
               num_list.append(col)
...
df_hig_iv = v2_cat_woe_iv(df_choose1,cols_list,target,0,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/")
df_hig_iv.to_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/high_iv_chi.xlsx")
...
#df_choose1.to_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/cut_bin_train.xlsx")
df_hig_iv = v2_cat_woe_iv(df_choose1,cols_list+str_columns,target,0,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/")
df_hig_iv.to_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/high_iv_chi.xlsx")
...
init_choose_name_desc.to_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/init_choose_name_desc_20190905v1.xlsx',index=False)
...
连续变量的相关性分析
"""
init_choose_name_desc=pd.read_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/init_choose_name_desc_20190905v1.xlsx')
init_choose_table=init_choose_name_desc.loc[init_choose_name_desc['是否选用'].isin(['是']),:]#手动去除一些不能用的变量
numeric_columns = init_choose_table.loc[init_choose_table.var_type.isin(['\ufeff数据型','数据型', '数字型','数值型']), 'var_name'].values.tolist()
str_columns = init_choose_table.loc[init_choose_table.var_type.isin(['字符型', '\ufeff字符型','字符型 ' ]), 'var_name'].values.tolist()
date_columns = init_choose_table.loc[init_choose_table.var_type.isin(['\ufeff日期型','日期型']), 'var_name'].values.tolist()
...
def main(df,col,address):
    ...
    for var in col:
        try:
          ...
          print(regroup)
          regroup.to_csv("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/num_bin_badrate.csv",mode="a",header=True)
        except:
            error_cols.append(var)
    df.to_csv(address+"data_woe_value_num.csv",header=True) # 包含分类变量和连续变量
    return error_cols,df

error_cols,df_choose3=main(df_choose2,col,'D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/')
...
df_high_cor1.to_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/df_high_cor.xlsx")


## 相关性去除iv更低的变量
cat_less_iv=pd.read_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/cat_less_iv_1.xlsx')
cat_less_iv=cat_less_iv.set_index('var_name').loc[str_columns,'IV'].reset_index().rename(columns={'IV':'iv'})
df_hig_iv=pd.read_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/high_iv_chi.xlsx")
df_hig_iv=pd.concat([df_hig_iv,cat_less_iv],axis=0)
df_hig_iv.to_csv('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/df_hig_iv_all.csv',index=False)
df_high_cor1=pd.read_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/df_high_cor.xlsx")
...
main(df_choose5,remain_col,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/final_test_num_bin_badrate1")
...
## 迭代，使得不在增加变量为止为停止准则
remain_col=['180overdue_times_Bin',
 'accu_loan_amt_Bin',
 'accu_settle_amt_Bin',
 'agreement_month_repay_Bin',
 'apt_age_Bin',
 'apt_ec_historyloantimes_Bin',
 'apt_ec_lastoverduedaystotal_Bin',
 'apt_ec_longestoverduedaystotallastyear_Bin',
 'apt_ec_nosettledcontractmoney_Bin',
 'apt_ec_overduephasetotallastyear_Bin',
 'apt_lastcreditamout_Bin',
 'apt_lastvehicle_evtrpt_evaluateprice_Bin',
 'd7_apply_platform_cnt_Bin',
 'd7_mobile_apply_num_Bin',
 'fh_execu_annu_num_Bin',
 'm12_apply_platform_cnt_mobile_Bin',
...
main(df_choose6,remain_col,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/final_test_num_bin_badrate2") 
...
vif_df.to_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/vif_df_20190905.xlsx')
...
remain_col=columns_coef.loc[columns_coef['coef']>0,'var_name'].values.tolist()

##最终剩下的变量
final_columns=['apt_ec_lastoverduedaystotal_Bin',
 'apt_ec_historyloantimes_Bin',
 'mate_work_place_info_date_Bin',
 'm12_apply_platform_cnt_mobile_Bin',
 'apt_ec_overduephasetotallastyear_Bin',
...
main(df_choose6,final_columns,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/final_test_num_bin_badrate3")  
...
for cn in final_columns:
    judge_stable_analyze(temp,cn,'D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/未调整分箱前的稳定性分析/','apply_y')
...
df_choose6['other_org_count_Bin']=df_choose6['other_org_count_Bin'].replace({'b-1NA':'NA&b1[0.0,1.0)',
    'b1[0.0,1.0)':'NA&b1[0.0,1.0)'})
...
df_choose6['180overdue_times_Bin']=df_choose6['180overdue_times_Bin'].replace({'b0[-inf, 2.0)':'notNA',
    'b1[2.0,14.0)':'notNA','b3[14.0,inf)':'notNA'})
df_choose6['apt_ec_historyloantimes_Bin']=df_choose6['apt_ec_historyloantimes_Bin'].replace({'b2[1.0,3.0)':'b2[1.0,inf)',
    'b3[3.0,4.0)':'b2[1.0,inf)','b4[4.0,inf)':'b2[1.0,inf)'})
...
df_choose6['apt_lastvehicle_evtrpt_evaluateprice_Bin']=df_choose6['apt_lastvehicle_evtrpt_evaluateprice_Bin'].replace({'b-1NA':'b-1NA&<50300',
    'b0[-inf, 50300.0)':'b-1NA&<50300','b1[50300.0,77500.0)':'b1[50300.0,121200.0)','b2[77500.0,121200.0)':'b1[50300.0,121200.0)'})
...
df_choose6['vehicle_evtrpt_mileage_Bin']=df_choose6['vehicle_evtrpt_mileage_Bin'].replace({'b1[16663.0,37678.0)':'b1[16663.inf)','b2[37678.0,77746.0)':'b1[16663.inf)','b2[37678.0,77746.0)':'b1[16663.inf)','b4[77746.0,inf)':'b1[16663.inf)'})
df_choose6['mate_work_place_info_date_Bin']=df_choose6['mate_work_place_info_date_Bin'].replace({'b-1NA':'b-1NA&b4[678.0,inf)','b4[678.0,inf)':'b-1NA&b4[678.0,inf)',
 'b0[-inf, 20.0)':'b0[-inf, 678.0)','b1[20.0,81.0)':'b0[-inf, 678.0)','b2[81.0,678.0)':'b0[-inf, 678.0)'})
df_choose6['m12_apply_platform_cnt_mobile_Bin']=df_choose6['m12_apply_platform_cnt_mobile_Bin'].replace({'b-1NA':'b-1NA&b0[-inf, 21)','b0[-inf, 2.0)':'b-1NA&b0[-inf, 21)',
 'b1[2.0,21.0)':'b-1NA&b0[-inf, 21)','b2[21.0,50.0)':'b2[21.0,inf)','b4[50.0,inf)':'b2[21.0,inf)'})
...
df_choose6['apt_age_Bin']=df_choose6['apt_age_Bin'].replace({'b0[-inf, 30.0)':'b0[-inf, 44.0)','b1[30.0,44.0)':'b0[-inf, 44.0)',
 'b-1NA':'b-1NA&b2[44.0,inf)','b2[44.0,50.0)':'b-1NA&b2[44.0,inf)','b4[50.0,inf)':'b-1NA&b2[44.0,inf)'})
df_choose6['apt_ec_longestoverduedaystotallastyear_Bin']=df_choose6['apt_ec_longestoverduedaystotallastyear_Bin'].replace({'b1[1.0,4.0)':'b1[1.0,inf)','b2[4.0,inf)':'b1[1.0,inf)'})
df_choose6['yx_underly_record_num_Bin']=np.nan
df_choose6.loc[df_choose1['yx_underly_record_num']>0,'yx_underly_record_num_Bin']='[1,inf)'
df_choose6.loc[df_choose1['yx_underly_record_num']<=0,'yx_underly_record_num_Bin']='NA&(-inf,1)'
df_choose6['max_overdue_amount_Bin']=np.nan
df_choose6.loc[df_choose1['max_overdue_amount']>0,'max_overdue_amount_Bin']='(0,inf)'
df_choose6.loc[df_choose1['max_overdue_amount']<=0,'max_overdue_amount_Bin']='NA&(-inf,0)'
  
    
main(df_choose6,final_columns,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/final_test_num_bin_badrate5")  
temp=df_choose6[final_columns+['apply_y']].copy()
temp['app_applydate']=model_data_new['app_applydate']
temp['applymonth']=temp['app_applydate'].str.slice(0,7)
for cn in final_columns:
    judge_stable_analyze(temp,cn,'D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/调整分箱后的稳定性分析/','apply_y')
...
choose_column.remove('apt_ec_longestoverduedaystotallastyear_Bin')
choose_column.remove('apt_ec_overduephasetotallastyear_Bin')
choose_column.remove('mate_work_place_info_date_Bin')
choose_column.remove('180overdue_times_Bin')
choose_column.remove('yx_underly_record_num_Bin')
choose_column.remove('apt_ec_historyloantimes_Bin')


lr=LogisticRegression(penalty='l1',C=1,n_jobs=-1,verbose=0,random_state=0)
...
#joblib.dump(lr,'D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/lr_alldata_20190906v1.pkl')
...
#columns_coef.to_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/columns_coef_20190906v1.xlsx',index=False)
...
r_p_chart2(y, pred_p3, min_scores, part=10)
...
def mate_work_place_info_date(x):
    if x == -99:
        return  -0.0337
    elif x < 20.0:
        return   0.4837
...
def m12_apply_platform_cnt_mobile(x):
    if x == -99:
        return  -0.1559
    elif x < 2.0:
        return   -0.1559
...
def apt_lastvehicle_evtrpt_evaluateprice(x):
    if x == -99:
        return    0.0521
    elif x < 50300.0:
        return    0.0521
...
def apt_ec_longestoverduedaystotallastyear(x):
    if x == -99:
        return    -0.0311
    elif x < 1.0:
        return     -0.0311
...
## 时间外验证进行重新评估
outtime_testdata=pd.read_table('outtime_testdata_20190906.txt',dtype={'app_applycode':str},sep='\u0001')
...
de_dict_var = pd.read_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/de_dict_vars_20190808.xlsx')
for i, _ in de_dict_var.iterrows():
    name = de_dict_var.loc[i, 'var_name']
    default = de_dict_var.loc[i, 'default']
...
for col in ['vehicle_minput_lastreleasedate','mate_work_place_info_date','email_info_date','apt_ec_lastloansettleddate']:  # 去除异常的时间
    try:
...
outtime_testdata["apt_ec_historyloantimes"+'_Bin'] = outtime_testdata["apt_ec_historyloantimes"].map(lambda x: apt_ec_historyloantimes(x))
outtime_testdata["mate_work_place_info_date"+'_Bin'] = outtime_testdata["mate_work_place_info_date"].map(lambda x: mate_work_place_info_date(x))
outtime_testdata["m12_apply_platform_cnt_mobile"+'_Bin'] = outtime_testdata["m12_apply_platform_cnt_mobile"].map(lambda x: m12_apply_platform_cnt_mobile(x))
outtime_testdata["apt_ec_overduephasetotallastyear"+'_Bin'] = outtime_testdata["apt_ec_overduephasetotallastyear"].map(lambda x: apt_ec_overduephasetotallastyear(x))
...
outtime_testdata["180overdue_times"+'_Bin'] = outtime_testdata["180overdue_times"].map(lambda x: _180overdue_times(x))
...
outtime_testdata["apt_age"+'_Bin'] = outtime_testdata["apt_age"].map(lambda x: apt_age(x))
outtime_testdata["apt_lastvehicle_evtrpt_evaluateprice"+'_Bin'] = outtime_testdata["apt_lastvehicle_evtrpt_evaluateprice"].map(lambda x: apt_lastvehicle_evtrpt_evaluateprice(x))
...
outtime_testdata["apt_ec_longestoverduedaystotallastyear"+'_Bin'] = outtime_testdata["apt_ec_longestoverduedaystotallastyear"].map(lambda x: apt_ec_longestoverduedaystotallastyear(x))
...
outtime_testdata["apt_gender"] = outtime_testdata["apt_gender"].map(lambda x: apt_gender(x))
outtime_testdata["vehicle_evtrpt_mileage"+'_Bin'] = outtime_testdata["vehicle_evtrpt_mileage"].map(lambda x: vehicle_evtrpt_mileage(x))
outtime_testdata["yx_underly_record_num"+'_Bin'] = outtime_testdata["yx_underly_record_num"].map(lambda x: yx_underly_record_num(x))
outtime_testdata["max_overdue_amount"+'_Bin'] = outtime_testdata["max_overdue_amount"].map(lambda x: max_overdue_amount(x))
...
for cn in choose_column:
    judge_stable_analyze(full_data,cn,'D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/调整分箱后的稳定性分析_加时间外验证/','apply_y')
