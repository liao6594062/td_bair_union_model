...
init_choose_name_desc=pd.read_excel('D:/llm/联合建模算法开发/百融联合建模_逻辑回归/init_choose_name_desc_20190808.xlsx')
...
连续变量的相关性分析
"""
init_choose_name_desc=pd.read_excel('D:/llm/联合建模算法开发/同盾联合建模_逻辑回归/init_choose_name_desc_20190809.xlsx')
init_choose_table=init_choose_name_desc.loc[init_choose_name_desc['是否选用'].isin(['是']),:]#手动去除一些不能用的变量
...
error_cols,df_choose3=main(df_choose2,col,'D:/llm/联合建模算法开发/同盾联合建模_逻辑回归/')
...
df_high_cor1.to_excel("D:/llm/联合建模算法开发/同盾联合建模_逻辑回归/df_high_cor.xlsx")


## 相关性去除iv更低的变量
cat_less_iv=pd.read_excel('D:/llm/联合建模算法开发/同盾联合建模_逻辑回归/cat_less_iv_1.xlsx')
cat_less_iv=cat_less_iv.set_index('var_name').loc[str_columns,'IV'].reset_index().rename(columns={'IV':'iv'})
df_hig_iv=pd.read_excel("D:/llm/联合建模算法开发/同盾联合建模_逻辑回归/high_iv_chi.xlsx")
df_hig_iv=pd.concat([df_hig_iv,cat_less_iv],axis=0)
df_hig_iv.to_csv('D:/llm/联合建模算法开发/同盾联合建模_逻辑回归/df_hig_iv_all.csv',index=False)
df_high_cor1=pd.read_excel("D:/llm/联合建模算法开发/同盾联合建模_逻辑回归/df_high_cor.xlsx")
...
main(df_choose5,remain_col,"D:/llm/联合建模算法开发/同盾联合建模_逻辑回归/final_test_num_bin_badrate1")
...
main(df_choose6,remain_col,"D:/llm/联合建模算法开发/同盾联合建模_逻辑回归/final_test_num_bin_badrate2") 
...
vif_df.to_excel('D:/llm/联合建模算法开发/同盾联合建模_逻辑回归/vif_df_20190814.xlsx')
...
df_hig_iv = v2_cat_woe_iv(df_choose7,final_columns,'apply_y',0.02,"D:/llm/联合建模算法开发/同盾联合建模_逻辑回归/")
...
df_choose6['apt_lastloanmode']=df_choose6['apt_lastloanmode'].map(str).replace({'2':'NA&123','3':'NA&123','-99':'NA&123'})
...
df_choose6['qtorg_query_orgcnt_Bin']=df_choose6['qtorg_query_orgcnt_Bin'].replace(
        {'b-1NA':'NA&<1','b1[0.0,1.0)':'NA&<1'}) 

main(df_choose6,final_columns,"D:/llm/联合建模算法开发/同盾联合建模_逻辑回归/final_test_num_bin_badrate5")  
df_hig_iv = v2_cat_woe_iv(df_choose6,final_columns,'apply_y',0.01,"D:/llm/联合建模算法开发/同盾联合建模_逻辑回归/")
...
#df_choose7.to_excel('D:/llm/联合建模算法开发/同盾联合建模_逻辑回归/训练集数据_20190806.xlsx')
...
#choose_column.remove('rational_score_Bin')



lr=LogisticRegression(penalty='l2',C=6,n_jobs=-1,verbose=0,random_state=0)
...
for cn in choose_column:
    judge_stable_analyze(full_data,cn,'D:/llm/联合建模算法开发/同盾联合建模_逻辑回归/调整分箱后的稳定性分析_加时间外验证/')
...
## 申请样本稳定性psi计算
psi_data=pd.read_table('psi_data_20190904.txt',dtype={'app_applycode':str},sep='\u0001')
psi_data=psi_data.replace('\\N',np.nan)
ylabel_data=pd.read_table('contractno_ylabel_data.txt',sep='\u0001',dtype={'applycode':str})  #车贷申请表
psi_data=pd.merge(psi_data,ylabel_data,left_on=['app_applycode','contractno'],right_on=['applycode','contractno'],how='left')
del ylabel_data
gc.collect()
outtime_testdata=outtime_testdata[outtime_testdata.apply_y.isin([0,1])].copy()
td_union_data=pd.read_excel('CarLoanData_BringToTounawang.xlsx')
psi_data['applydate']=psi_data['app_applydate'].str.slice(0,10)
psi_data=pd.merge(psi_data,td_union_data,left_on=['credentials_no_md5','cust_name_md5','mobile_md5','applydate'],right_on=
                     ['credentials_no_md5_x','cust_name_md5_x','mobile_md5_x','applydate'],how='left')
del td_union_data
gc.collect()
outtime_testdata.columns=[cn.lower() for cn in outtime_testdata.columns]

psi_data=pd.merge(psi_data,final_data[['app_applycode','apply_y']],on='app_applycode',how='left')
