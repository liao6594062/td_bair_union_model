...
init_choose_table=init_choose_table[init_choose_table['数据源'].isin(['百融联合建模'])==False]#去掉同盾联合建模变量
numeric_columns = init_choose_table.loc[init_choose_table.var_type.isin(['\ufeff数据型','数据型', '数字型','数值型']), 'var_name'].values.tolist()
str_columns = init_choose_table.loc[init_choose_table.var_type.isin(['字符型', '\ufeff字符型','字符型 ' ]), 'var_name'].values.tolist()
date_columns = init_choose_table.loc[init_choose_table.var_type.isin(['\ufeff日期型','日期型']), 'var_name'].values.tolist()
print(len(numeric_columns)+len(str_columns)+len(date_columns))
...
def main(df,col,address):
    ...
    for var in col:
        try:
          ...
          print(regroup)
          regroup.to_csv("D:/llm/联合建模算法开发/自有数据_逻辑回归/num_bin_badrate.csv",mode="a",header=True)
        except:
            error_cols.append(var)
    df.to_csv(address+"data_woe_value_num.csv",header=True) # 包含分类变量和连续变量
    return error_cols,df

error_cols,df_choose3=main(df_choose2,col,'D:/llm/联合建模算法开发/自有数据_逻辑回归/')
...
df_high_cor1.to_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归/df_high_cor.xlsx")


## 相关性去除iv更低的变量
cat_less_iv=pd.read_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归/cat_less_iv_1.xlsx')
cat_less_iv=cat_less_iv.set_index('var_name').loc[str_columns,'IV'].reset_index().rename(columns={'IV':'iv'})
df_hig_iv=pd.read_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归/high_iv_chi.xlsx")
df_hig_iv=pd.concat([df_hig_iv,cat_less_iv],axis=0)
df_hig_iv.to_csv('D:/llm/联合建模算法开发/自有数据_逻辑回归/df_hig_iv_all.csv',index=False)
df_high_cor1=pd.read_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归/df_high_cor.xlsx")
...
main(df_choose5,remain_col,"D:/llm/联合建模算法开发/自有数据_逻辑回归/final_test_num_bin_badrate1")
...
main(df_choose6,remain_col,"D:/llm/联合建模算法开发/自有数据_逻辑回归/final_test_num_bin_badrate2") 
...
vif_df.to_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归/vif_df_20190813.xlsx')
...
df_hig_iv = v2_cat_woe_iv(df_choose7,final_columns,'apply_y',0.02,"D:/llm/联合建模算法开发/自有数据_逻辑回归/")
df_hig_iv=df_hig_iv.reset_index().rename(columns={'index':'var_name',0.0:'iv'}).sort_values('iv',ascending=False)


##最终剩下的变量
final_columns=['mate_mobile_info_date_Bin',
 'apt_ec_overduephasetotallastyear_Bin',
 'contacts_router_ratio_Bin',
 'contacts_class1_cnt_Bin',
 'contact_bank_call_cnt_Bin',
 ...
 'max_call_in_cnt_l6m_Bin',
 'prof_title_info_date_Bin',
 'max_overdue_terms_Bin','cell_reg_time_Bin']
...
df_choose6['max_total_amount_l6m_Bin']=df_choose6['max_total_amount_l6m_Bin'].replace({'b2[298.8,462.91)':'b2[298.8,inf)',
    'b-1NA':'NA&b0[-inf, 298.8)','b4[462.91,inf)':'b2[298.8,inf)','b0[-inf, 110.39)':'NA&b0[-inf, 298.8)','b1[110.39,298.8)':'NA&b0[-inf, 298.8)'})
...
df_choose6['contacts_class1_cnt_Bin']=df_choose6['contacts_class1_cnt_Bin'].replace({'b-1NA':'NA&b1[-inf,371.0)',
    'b1[20.0,371.0)':'NA&b1[-inf,371.0)','b2[371.0,461.0)':'b2[371.0,inf)','b4[461.0,inf)':'b2[371.0,inf)','b0[-inf, 20.0)':'NA&b1[-inf,371.0)'})
...
#df_choose6['jxl_id_comb_othertel_num_Bin']=df_choose6['jxl_id_comb_othertel_num_Bin'].replace({'b2[2.0,3.0)':'b2[2.0,inf)',
#    'b4[3.0,inf)':'b2[2.0,inf)'})
    
main(df_choose6,final_columns,"D:/llm/联合建模算法开发/自有数据_逻辑回归/final_test_num_bin_badrate4")  
df_hig_iv = v2_cat_woe_iv(df_choose6,final_columns,'apply_y',0.01,"D:/llm/联合建模算法开发/自有数据_逻辑回归/")
...
for cn in choose_column:
    judge_stable_analyze(full_data,cn,'D:/llm/联合建模算法开发/自有数据_逻辑回归/调整分箱后的稳定性分析_加时间外验证/')
