# -*- coding: utf-8 -*-
"""
Created on Mon Sep  9 14:32:28 2019
...
from c_tools  import v2_cat_woe_iv,v2_equif_bin,judge_stable_analyze
...
#de_dict.to_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/DE_Var_Select_20190808.xlsx',index=False)
...
de_dict_var = pd.read_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/DE_Var_Select_20190808.xlsx')
...
#de_dict_vars.to_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/de_dict_vars_20190808.xlsx',index=False)
del de_dict_vars
...
vars_count_table=pd.read_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/de_dict_vars_20190909.xlsx') #手动增加了同盾外部评分
...
model_data_new=combine_data.loc[combine_data.app_applydate<='2018-10-31 23:59:59',choose_columns_table.var_name.tolist()+
                               ['apply_y','app_applydate','vehicle_illegal_avgpenaltypoints','vehicle_insurance_claimsavgamount','age_and_gender','province_consume']].copy()
...
#    cat_less20 = ["deci_apt_facetrial_housetype","deci_cont02_relationship","deci_jxl_contact2_rel","deci_vehicle_minput_lastmortgagerinfo"]
    high_iv_name = v2_cat_woe_iv(df,cat_less20,target,0,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/") # 计算woe 和IV值
    group = []
    ...
    print(less_iv_name)
    less_iv_name.to_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/cat_less_iv_1.xlsx")
    df = v2_equif_bin(df, cat_over20, target)
    cols_name = [i+"_Bin" for i in cat_over20]
    over_iv_name = v2_cat_woe_iv(df, cols_name, target,0,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/")  # 计算woe 和IV值
    over_iv_name.to_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/cat_more_iv_2.xlsx")
...
#init_choose_name_desc.to_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/init_choose_name_desc_20190909.xlsx',index=False)
init_choose_name_desc=pd.read_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/init_choose_name_desc_20190909.xlsx')
    ...
    for col in num_var:
        ...
        if -99 not in set(df[col]):
            ...
            if len(cutOff)!=0:
               ...
               regroup.to_csv("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/regroup_Chi_cutbin__.csv", mode="a", header=True)
               print(regroup)
               num_list.append(col)
...
df_hig_iv = v2_cat_woe_iv(df_choose1,cols_list,target,0,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/")
df_hig_iv.to_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/high_iv_chi.xlsx")
...
df_choose1['d1_id_relate_device_num_Bin']=df_choose1['d1_id_relate_device_num_Bin'].replace({'b2[0.0,1.0)':'notNA','b4[1.0,inf)':'notNA'})
...
df_choose1['vehicle_evtrpt_evaluatedate_Bin']=df_choose1['vehicle_evtrpt_evaluatedate_Bin'].replace({'b-1NA':'b-1NA&b0[-inf, 0.0)','b0[-inf, 0.0)':'b-1NA&b0[-inf, 0.0)'})
df_choose1['vehicle_evtrpt_evaluateprice_Bin']=df_choose1['vehicle_evtrpt_evaluateprice_Bin'].replace({'b-1NA':'b-1NA&b0[-inf, 41400.0)','b0[-inf, 41400.0)':'b-1NA&b0[-inf, 41400.0)'})
df_choose1['vehicle_buymode']=df_choose1['vehicle_buymode'].replace(-99,1)
df_choose1['vehicle_illegalis']=df_choose1['vehicle_illegalis'].replace(-99,0)
df_choose1['vehicle_minput_ownerway']=df_choose1['vehicle_minput_ownerway'].isin([1]).astype('float64')
df_choose1['vehicle_minput_registcertflag']=df_choose1['vehicle_minput_registcertflag'].replace(-99,2)
#df_choose1.to_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/cut_bin_train.xlsx")
df_hig_iv = v2_cat_woe_iv(df_choose1,cols_list+str_columns,target,0,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/")
df_hig_iv.to_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/high_iv_chi.xlsx")
...
iv_name1.remove('vehicle_minput_driverlicenseflag')
init_choose_name_desc=choose_columns_table.set_index('var_name').loc[iv_name1,:].reset_index()
init_choose_name_desc.to_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/init_choose_name_desc_20190909v1.xlsx',index=False)
...
连续变量的相关性分析
"""
init_choose_name_desc=pd.read_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/init_choose_name_desc_20190909v1.xlsx')
init_choose_table=init_choose_name_desc.loc[init_choose_name_desc['是否选用'].isin(['是']),:]#手动去除一些不能用的变量
numeric_columns = init_choose_table.loc[init_choose_table.var_type.isin(['\ufeff数据型','数据型', '数字型','数值型']), 'var_name'].values.tolist()
str_columns = init_choose_table.loc[init_choose_table.var_type.isin(['字符型', '\ufeff字符型','字符型 ' ]), 'var_name'].values.tolist()
date_columns = init_choose_table.loc[init_choose_table.var_type.isin(['\ufeff日期型','日期型']), 'var_name'].values.tolist()
...
def main(df,col,address):
    ...
    for var in col:
        try:
          ...
          print(regroup)
          regroup.to_csv("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/num_bin_badrate.csv",mode="a",header=True)
        except:
            error_cols.append(var)
    df.to_csv(address+"data_woe_value_num.csv",header=True) # 包含分类变量和连续变量
    return error_cols,df

error_cols,df_choose3=main(df_choose2,col,'D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/')
...
df_high_cor1.to_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/df_high_cor.xlsx")


## 相关性去除iv更低的变量
cat_less_iv=pd.read_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/cat_less_iv_1.xlsx')
cat_less_iv=cat_less_iv.set_index('var_name').loc[str_columns,'IV'].reset_index().rename(columns={'IV':'iv'})
df_hig_iv=pd.read_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/high_iv_chi.xlsx")
df_hig_iv=pd.concat([df_hig_iv,cat_less_iv],axis=0)
df_hig_iv.to_csv('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/df_hig_iv_all.csv',index=False)
df_high_cor1=pd.read_excel("D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/df_high_cor.xlsx")
...
main(df_choose5,remain_col,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/final_test_num_bin_badrate1")
...
remain_col=['accu_settle_amt_Bin',
 'app_sameaddrhitnum_Bin',
 'app_samenamehitnum_Bin',
 'apt_ec_nosettledcontractmoney_Bin',
 ...
 'vehicle_minput_seats_Bin',
 'yx_underly_record_num_Bin',
 'apt_ec_lastloansettleddate_Bin',
 'email_info_date_Bin',
 'mate_work_place_info_date_Bin',
 'vehicle_jinsuranceendtime_Bin',
 'vehicle_minput_drivinglicensevaliditydate_Bin',
 'vehicle_minput_lastreleasedate_Bin',
 'vehicle_sinsuranceendtime_Bin',
 'age_and_gender',
 'apt_facetrial_housetype',
 'apt_lastloanmode',
 'vehicle_buymode',
 'vehicle_evtrpt_fueltype',
 'vehicle_evtrpt_maker',
 'vehicle_minput_attribution',
 'vehicle_minput_driverlicense',
...
main(df_choose6,remain_col,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/final_test_num_bin_badrate2") 
...
vif_df.to_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/vif_df_20190909.xlsx')
...
df_hig_iv = v2_cat_woe_iv(df_choose7,final_columns,'apply_y',0,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/")
df_hig_iv=df_hig_iv.reset_index().rename(columns={'index':'var_name',0.0:'iv'}).sort_values('iv',ascending=False)


##最终剩下的变量
final_columns=['app_sameaddrhitnum_Bin',
 'vehicle_buymode',
 'vehicle_minput_ownerway',
 'vehicle_evtrpt_maker',
 'vehicle_evtrpt_b2bprice_Bin',
...
main(df_choose6,final_columns,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/final_test_num_bin_badrate3")  
temp=df_choose6[final_columns+['apply_y']].copy()
temp['app_applydate']=model_data_new['app_applydate']
temp['applymonth']=temp['app_applydate'].str.slice(0,7)
for cn in final_columns:
    judge_stable_analyze(temp,cn,'D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/未调整分箱前的稳定性分析/','apply_y')

##去除不稳定的变量
final_columns.remove('app_sameaddrhitnum_Bin')
final_columns.remove('overdue_times_Bin')
final_columns.remove('vehicle_evtrpt_fueltype')
...
## 针对筛选出来的变量进行分箱并调整
df_choose6=df_choose1[final_columns+['apply_y']].copy()
df_choose6['apt_lastloanmode']=df_choose6['apt_lastloanmode'].astype('int64').astype('str').replace({'2':'NA&123','3':'NA&123','-99':'NA&123'})
df_choose6['max_overdue_terms_Bin']=df_choose6['max_overdue_terms_Bin'].replace({'b2[1.0,2.0)':'b2[1.0,inf)',
    'b4[2.0,inf)':'b2[1.0,inf)','b1[0.0,1.0)':'NA&<1','b-1NA':'NA&<1'})         
...
df_choose6['m3_apply_mobile_platform_cnt_Bin']=df_choose6['m3_apply_mobile_platform_cnt_Bin'].replace({
    'b1[0.0,8.0)':'notNA','b2[8.0,26.0)':'notNA','b4[26.0,inf)':'notNA'})
df_choose6['apt_ec_lastloansettleddate_Bin']=df_choose6['apt_ec_lastloansettleddate_Bin'].replace({'b-1NA':'b-1NA&<0',
    'b0[-inf, -1.0)':'b-1NA&<0','b1[-1.0,0.0)':'b-1NA&<0','b2[0.0,72.0)':'b2[0.0,inf)','b4[72.0,inf)':'b2[0.0,inf)'})
df_choose6['email_info_date_Bin']=df_choose6['email_info_date_Bin'].replace({'b0[-inf, -29.0)':'b0[-inf,424.0)',
    'b1[-29.0,96.0)':'b0[-inf,424.0)','b2[96.0,424.0)':'b0[-inf,424.0)','b-1NA':'b-1NA&b4[424.0,inf)','b4[424.0,inf)':'b-1NA&b4[424.0,inf)'})
df_choose6['m3_id_relate_mobile_num_Bin']=df_choose6['m3_id_relate_mobile_num_Bin'].replace({'b-1NA':'b-1NA&b1[0.0,2.0)','b1[0.0,2.0)':'b-1NA&b1[0.0,2.0)'})
df_choose6['apt_ec_overduephasetotallastyear_Bin']=df_choose6['apt_ec_overduephasetotallastyear_Bin'].replace({'b1[1.0,2.0)':'b1[1.0,inf)','b2[2.0,3.0)':'b1[1.0,inf)',
 'b3[3.0,4.0)':'b1[1.0,inf)','b4[4.0,inf)':'b1[1.0,inf)'})
df_choose6['vehicle_evtrpt_b2bprice_Bin']=df_choose6['vehicle_evtrpt_b2bprice_Bin'].replace({'b-1NA':'b-1NA&b0[-inf, 4.3)','b0[-inf, 3.33)':'b-1NA&b0[-inf, 4.3)',
 'b1[3.33,4.3)':'b-1NA&b0[-inf, 4.3)','b2[4.3,10.6)':'b2[4.3,inf)','b4[10.6,inf)':'b2[4.3,inf)'})
df_choose6['qtorg_query_orgcnt_Bin']=df_choose6['qtorg_query_orgcnt_Bin'].replace({'b-1NA':'b-1NA&b1[0.0,1.0)','b1[0.0,1.0)':'b-1NA&b1[0.0,1.0)'})        
df_choose6['vehicle_jinsuranceendtime_Bin']=df_choose6['vehicle_jinsuranceendtime_Bin'].replace({'b-1NA':'b-1NA&b0[-inf, -301.0)','b0[-inf, -363.0)':'b-1NA&b0[-inf, -301.0)',
    'b1[-363.0,-301.0)':'b-1NA&b0[-inf, -301.0)','b2[-301.0,-110.0)':'b2[-301.0,inf)','b4[-110.0,inf)':'b2[-301.0,inf)'})   
df_choose6['province_consume_Bin']=df_choose6['province_consume_Bin'].replace({'b-1NA':'b-1NA&b4[69062.0,inf)','b4[69062.0,inf)':'b-1NA&b4[69062.0,inf)',
    'b0[-inf, 65726.0)':'b0[-inf, 69062.0)','b1[65726.0,67927.0)':'b0[-inf, 69062.0)','b2[67927.0,69062.0)':'b0[-inf, 69062.0)'})   
df_choose6['vehicle_minput_drivinglicensevaliditydate_Bin']=np.nan
df_choose6.loc[df_choose1['vehicle_minput_drivinglicensevaliditydate']>0,'vehicle_minput_drivinglicensevaliditydate_Bin']='NA&(0,inf)'
df_choose6.loc[df_choose1['vehicle_minput_drivinglicensevaliditydate']<=0,'vehicle_minput_drivinglicensevaliditydate_Bin']='(-inf,0]'
df_choose6.loc[df_choose1['vehicle_minput_drivinglicensevaliditydate']==-99,'vehicle_minput_drivinglicensevaliditydate_Bin']='NA&(0,inf)'
df_choose6['vehicle_minput_attribution']=df_choose6['vehicle_minput_attribution'].astype('int64').map(str).replace({'-99':'-99&=1','1':'-99&=1','2':'2&3','3':'2&3'})     
df_choose6['vehicle_illegal_payamt_Bin']=df_choose6['vehicle_illegal_payamt_Bin'].replace({'b-1NA':'b-1NA&b0[-inf, 1500)','b0[-inf, 200.0)':'b-1NA&b0[-inf, 1500)','b1[200.0,1500.0)':'b-1NA&b0[-inf, 1500)',
    'b2[1500.0,2250.0)':'b2[1500.0,inf)','b4[2250.0,inf)':'b2[1500.0,inf)'})
df_choose6['vehicle_minput_lastmortgagerinfo']=df_choose6['vehicle_minput_lastmortgagerinfo'].astype('int64').map(str).replace({'-99':'-99&3&4','3':'-99&3&4','4':'-99&3&4','1':'1&2&5','2':'1&2&5','5':'1&2&5'})
df_choose6['apt_facetrial_housetype']=df_choose6['apt_facetrial_housetype'].astype('int64').map(str).replace({'1':'1&2','2':'1&2','3':'3&4&5&6','4':'3&4&5&6','5':'3&4&5&6','6':'3&4&5&6'})
 
    
main(df_choose6,final_columns,"D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/final_test_num_bin_badrate4")  
temp=df_choose6[final_columns+['apply_y']].copy()
temp['app_applydate']=model_data_new['app_applydate']
temp['applymonth']=temp['app_applydate'].str.slice(0,7)
for cn in final_columns:
    judge_stable_analyze(temp,cn,'D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/调整分箱后的稳定性分析/','apply_y')
choose_column=final_columns.copy()
...
#df_choose7.to_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立/训练集数据_20190806.xlsx')
...
result=lgt.fit()
print(result.summary2())
#choose_column.remove('apt_ec_lastloansettleddate_Bin')
choose_column.remove('vehicle_minput_lastmortgagerinfo')
choose_column.remove('vehicle_minput_registcertflag')
choose_column.remove('vehicle_minput_drivinglicensevaliditydate_Bin')
choose_column.remove('m3_id_relate_mobile_num_Bin')
#choose_column.remove('vehicle_minput_attribution')
choose_column.remove('vehicle_jinsuranceendtime_Bin')
choose_column.remove('apt_facetrial_housetype')
choose_column.remove('vehicle_illegal_payamt_Bin')
...
#joblib.dump(lr,'D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/lr_alldata_20190911v1.pkl')
...
#columns_coef.to_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/columns_coef_20190911v1.xlsx',index=False)
...
def m3_apply_mobile_platform_cnt(x):
    if x == -99:
        return  -0.4487
    elif x < 8.0:
        return   0.1125
...
def vehicle_minput_drivinglicensevaliditydate(x):
    if x == -99:
        return    -0.0332
    elif x<= 0:
        return    0.6448
...
## 时间外验证集数据的提取与处理
outtime_testdata=pd.read_table('outtime_testdata_20190910.txt',dtype={'app_applycode':str},sep='\u0001')
...
consume_province=pd.read_excel('统计局数据_省级.xlsx',sheetname='各省平均工资（元）')  ##衍生一个收入变量
outtime_testdata=pd.merge(outtime_testdata,consume_province[['地区','2017年']],left_on='app_siteprovince',right_on='地区',how='left')
outtime_testdata=outtime_testdata.rename(columns={'2017年':'province_consume'})
del consume_province
gc.collect()

de_dict_var = pd.read_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/de_dict_vars_20190909.xlsx')
for i, _ in de_dict_var.iterrows():
    name = de_dict_var.loc[i, 'var_name']
    default = de_dict_var.loc[i, 'default']
...
for col in ['vehicle_minput_lastreleasedate','mate_work_place_info_date','email_info_date','apt_ec_lastloansettleddate',
            'vehicle_minput_drivinglicensevaliditydate','vehicle_jinsuranceendtime']:  # 去除异常的时间
    try:
...
for col in ['vehicle_minput_lastreleasedate','mate_work_place_info_date','email_info_date','apt_ec_lastloansettleddate',
            'vehicle_minput_drivinglicensevaliditydate','vehicle_jinsuranceendtime']:
    if col != 'app_applydate':
        try:
            if col not in ['vehicle_minput_drivinglicensevaliditydate']:
...
## 变量衍生
outtime_testdata['age_and_gender']=1
outtime_testdata.loc[(outtime_testdata.apt_age>=43) & (outtime_testdata.apt_gender.isin(['男'])),'age_and_gender']=0  ##年龄与性别组合
outtime_testdata.loc[(outtime_testdata.apt_gender.isin(['女'])),'age_and_gender']=0           

## woe转换
outtime_testdata=outtime_testdata.fillna(-99)
outtime_testdata=outtime_testdata[outtime_testdata.app_applydate<='2018-11-30 23:59:59']
outtime_testdata["vehicle_buymode"] = outtime_testdata["vehicle_buymode"].map(lambda x: vehicle_buymode(x))
outtime_testdata["vehicle_minput_ownerway"] = outtime_testdata["vehicle_minput_ownerway"].map(lambda x: vehicle_minput_ownerway(x))
...
outtime_testdata["email_info_date"+'_Bin'] = outtime_testdata["email_info_date"].map(lambda x: email_info_date(x))
outtime_testdata["vehicle_minput_registcertflag"] = outtime_testdata["vehicle_minput_registcertflag"].map(lambda x: vehicle_minput_registcertflag(x))
outtime_testdata['vehicle_jinsuranceendtime'+'_Bin'] = outtime_testdata["vehicle_jinsuranceendtime"].map(lambda x: vehicle_jinsuranceendtime(x))
outtime_testdata["m3_id_relate_mobile_num"+'_Bin'] = outtime_testdata["m3_id_relate_mobile_num"].map(lambda x: m3_id_relate_mobile_num(x))
outtime_testdata["max_overdue_terms"+'_Bin'] = outtime_testdata["max_overdue_terms"].map(lambda x: max_overdue_terms(x))
outtime_testdata["province_consume"+'_Bin'] = outtime_testdata["province_consume"].map(lambda x: province_consume(x))
outtime_testdata["vehicle_minput_drivinglicensevaliditydate"+'_Bin'] = outtime_testdata["vehicle_minput_drivinglicensevaliditydate"].map(lambda x: vehicle_minput_drivinglicensevaliditydate(x))
outtime_testdata["age_and_gender"] = outtime_testdata["age_and_gender"].map(lambda x: age_and_gender(x))
outtime_testdata['vehicle_minput_attribution'] = outtime_testdata["vehicle_minput_attribution"].map(lambda x: vehicle_minput_attribution(x))
outtime_testdata["vehicle_illegal_payamt"+'_Bin'] = outtime_testdata["vehicle_illegal_payamt"].map(lambda x: vehicle_illegal_payamt(x))
outtime_testdata["apt_ec_lastloansettleddate"+'_Bin'] = outtime_testdata["apt_ec_lastloansettleddate"].map(lambda x: apt_ec_lastloansettleddate(x))
...
outtime_testdata["apt_facetrial_housetype"] = outtime_testdata["apt_facetrial_housetype"].map(lambda x: apt_facetrial_housetype(x))
...
print('时间外验证')
outtime_testdata1=pd.concat([outtime_testdata[choose_column+['apply_y']],testdata[choose_column+['apply_y']]])
pred_p4= lr.predict_proba(outtime_testdata1[choose_column])[:, 1]
fpr, tpr, th = roc_curve(outtime_testdata1.apply_y, pred_p4)
ks3 = tpr - fpr
print('all ks:   ' + str(max(ks3)))
print(roc_auc_score(outtime_testdata1.apply_y, pred_p4))
r_p_chart2(outtime_testdata1.apply_y, pred_p4, min_scores, part=20)


'''
##6.3 模型评估-其他指标（psi、ks、auc、gini)
    按月统计；
    按数据集统计；
    模型对比分析等；
'''
...
'''
##6.3.1 入模变量在时间外验证集上的稳定性
'''
df_choose7=df_choose6[choose_column+['apply_y']].copy()
df_choose7[['app_applydate',]]=model_data_new[['app_applydate']].copy()
testdata=df_choose7[df_choose7.app_applydate>='2018-10-01 00:00:00']
full_data=pd.concat([testdata[choose_column+['apply_y','app_applydate']],outtime_testdata[choose_column+['apply_y','app_applydate']]])
#full_data.to_csv('D:/llm/联合建模算法开发/百融联合建模_逻辑回归/full_data_20190722.csv',index=False)
for cn in choose_column:
    judge_stable_analyze(full_data,cn,'D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/调整分箱后的稳定性分析_加时间外验证/','apply_y')
...
full_data['own_scores']=pred_p5*100
...
for cn in full_data['applymonth'].unique():
    temp=full_data.loc[full_data['applymonth']==cn,['apply_y','own_scores']].copy()
...
'''
##6.3.3 计算模型在时间外样本集上的psi
'''

'''
##6.3.3-1 计算全部申请样本的模型分数
'''
psi_data=pd.read_table('OwnModel_psi_data_20190901.txt',dtype={'app_applycode':str},sep='\u0001')
psi_data=psi_data.replace('\\N',np.nan)
train_data=df_choose6[choose_column+['apply_y']].copy()
model_data_new=combine_data.loc[combine_data.app_applydate<='2018-10-31 23:59:59',['apply_y','app_applydate','app_applycode']].copy()
train_data[['app_applydate','app_applycode']]=model_data_new[['app_applydate','app_applycode']].copy()
train_data=train_data[train_data.app_applydate<='2018-09-30 23:59:59']
psi_data=pd.merge(psi_data,train_data[['app_applycode','apply_y']],on='app_applycode',how='left')
del train_data
gc.collect()
psi_data.columns=[cn.lower() for cn in psi_data.columns]

consume_province=pd.read_excel('统计局数据_省级.xlsx',sheetname='各省平均工资（元）')  ##衍生一个收入变量
psi_data=pd.merge(psi_data,consume_province[['地区','2017年']],left_on='app_siteprovince',right_on='地区',how='left')
psi_data=psi_data.rename(columns={'2017年':'province_consume'})
del consume_province
gc.collect()

psi_data.loc[psi_data.app_applydate>='2018-12-18 00:00:00','m3_apply_mobile_platform_cnt']=psi_data.loc[psi_data.app_applydate>='2018-12-18 00:00:00','m3_pho_mulplatloan_allnum'].copy()
de_dict_var = pd.read_excel('D:/llm/联合建模算法开发/自有数据_逻辑回归_剔除聚信立_增加若干车辆信息/de_dict_vars_20190909.xlsx')
for i, _ in de_dict_var.iterrows():
    name = de_dict_var.loc[i, 'var_name']
    default = de_dict_var.loc[i, 'default']
...
## 变量衍生
psi_data['age_and_gender']=1
psi_data.loc[(psi_data.apt_age>=43) & (psi_data.apt_gender.isin(['男'])),'age_and_gender']=0  ##年龄与性别组合
psi_data.loc[(psi_data.apt_gender.isin(['女'])),'age_and_gender']=0    


psi_data=psi_data.fillna(-99)
psi_data["vehicle_buymode"] = psi_data["vehicle_buymode"].map(lambda x: vehicle_buymode(x))
psi_data["vehicle_minput_ownerway"] = psi_data["vehicle_minput_ownerway"].map(lambda x: vehicle_minput_ownerway(x))
psi_data["vehicle_evtrpt_b2bprice"+'_Bin'] = psi_data["vehicle_evtrpt_b2bprice"].map(lambda x: vehicle_evtrpt_b2bprice(x))
psi_data["qtorg_query_orgcnt"+'_Bin'] = psi_data["qtorg_query_orgcnt"].map(lambda x: qtorg_query_orgcnt(x))
psi_data["apt_ec_overduephasetotallastyear"+'_Bin'] = psi_data["apt_ec_overduephasetotallastyear"].map(lambda x: apt_ec_overduephasetotallastyear(x))
...
#psi_data["vehicle_minput_registcertflag"] = psi_data["vehicle_minput_registcertflag"].map(lambda x: vehicle_minput_registcertflag(x))
#psi_data['vehicle_jinsuranceendtime'+'_Bin'] = psi_data["vehicle_jinsuranceendtime"].map(lambda x: vehicle_jinsuranceendtime(x))
#psi_data["m3_id_relate_mobile_num"+'_Bin'] = psi_data["m3_id_relate_mobile_num"].map(lambda x: m3_id_relate_mobile_num(x))
psi_data["max_overdue_terms"+'_Bin'] = psi_data["max_overdue_terms"].map(lambda x: max_overdue_terms(x))
psi_data["province_consume"+'_Bin'] = psi_data["province_consume"].map(lambda x: province_consume(x))
#psi_data["vehicle_minput_drivinglicensevaliditydate"+'_Bin'] = psi_data["vehicle_minput_drivinglicensevaliditydate"].map(lambda x: vehicle_minput_drivinglicensevaliditydate(x))
psi_data["age_and_gender"] = psi_data["age_and_gender"].map(lambda x: age_and_gender(x))
psi_data['vehicle_minput_attribution'] = psi_data["vehicle_minput_attribution"].map(lambda x: vehicle_minput_attribution(x))
#psi_data["vehicle_illegal_payamt"+'_Bin'] = psi_data["vehicle_illegal_payamt"].map(lambda x: vehicle_illegal_payamt(x))
psi_data["apt_ec_lastloansettleddate"+'_Bin'] = psi_data["apt_ec_lastloansettleddate"].map(lambda x: apt_ec_lastloansettleddate(x))
psi_data["m3_apply_mobile_platform_cnt"+'_Bin'] = psi_data["m3_apply_mobile_platform_cnt"].map(lambda x: m3_apply_mobile_platform_cnt(x))
#psi_data["vehicle_minput_lastmortgagerinfo"] = psi_data["vehicle_minput_lastmortgagerinfo"].map(lambda x: vehicle_minput_lastmortgagerinfo(x))
...
#psi_data['apt_lastloanmode'] = psi_data["apt_lastloanmode"].map(lambda x: apt_lastloanmode(x))
#psi_data["apt_facetrial_housetype"] = psi_data["apt_facetrial_housetype"].map(lambda x: apt_facetrial_housetype(x))
...
'''
##6.3.3-2 计算在建模期申请样本集上的模型区分能力以及20分段上的客户占比
'''

#模型区分能力
modeltime_applydata=psi_data[(pd.to_datetime(psi_data.app_applydate)>=pd.to_datetime('2018-03-01 00:00:00')) & (pd.to_datetime(psi_data.app_applydate)<=pd.to_datetime('2018-09-30 23:59:59'))]
modeltime_applydata.loc[modeltime_applydata.apply_y.isnull(),'apply_y']=0
r_p_chart2(modeltime_applydata.apply_y, (modeltime_applydata.newloan_score3.values/100).tolist(), min_scores, part=20)
fpr, tpr, th = roc_curve(modeltime_applydata.apply_y, (modeltime_applydata.newloan_score3.values/100).tolist())
ks3 = tpr - fpr
print('all ks:   ' + str(max(ks3)))

## 各分段的占比
modeltime_apply_num=pd.cut(modeltime_applydata.newloan_score3,cuts,right=False).value_counts().sort_index(ascending=False)
modeltime_apply_num=modeltime_apply_num/modeltime_apply_num.sum()
print(modeltime_apply_num)


## 时间外样本
outtime_psidata=modeltime_applydata=psi_data[(pd.to_datetime(psi_data.app_applydate)>=pd.to_datetime('2018-10-01 00:00:00')) & (pd.to_datetime(psi_data.app_applydate)<=pd.to_datetime('2018-11-30 23:59:59'))]
...
'''
## 模型评估-模型比较（与修改聚信立默认值后的申请评分三期的比较）
'''

applymodel3=pd.read_excel('聚信立设置默认值的三期模型分数_201810-11.xlsx') ##需要先计算出来
model_data_new=combine_data.loc[combine_data.app_applydate<='2018-10-31 23:59:59',choose_columns_table.var_name.tolist()+
                               ['apply_y','app_applydate','app_applycode']].copy()
df_choose7=df_choose6[choose_column+['apply_y']].copy()
df_choose7[['app_applydate','app_applycode']]=model_data_new[['app_applydate','app_applycode']].copy()
testdata=df_choose7[df_choose7.app_applydate>='2018-10-01 00:00:00']
full_data=pd.concat([testdata[choose_column+['apply_y','app_applydate','app_applycode']],outtime_testdata[choose_column+['apply_y','app_applydate','app_applycode']]])
compare_data=pd.merge(full_data,applymodel3,on='app_applycode',how='inner')

## 新模型的模型效果
min_scores=[0.56659,0.49232,0.43571,0.40295,0.36865,0.34011,0.32176,0.30356,0.28667,0.26578,
            0.25262,0.24123,0.22877,0.21407,0.19957,0.17692,0.1676,0.15041,0.12509,0]
pred_p6= lr.predict_proba(compare_data[choose_column])[:, 1]
fpr, tpr, th = roc_curve(compare_data.apply_y, pred_p6)
ks3 = tpr - fpr
print('all ks:   ' + str(max(ks3)))
print(roc_auc_score(compare_data.apply_y, pred_p6))
r_p_chart2(compare_data.apply_y, pred_p6, min_scores, part=20)

## 修改聚信立默认值后的申请评分三期的模型效果
min_scores3=[0.2572,0.20455,0.17644,0.15728,0.1424,0.13058,0.12107,0.11313,0.10629,0.10051,
            0.09563,0.09117,0.08736,0.08384,0.08036,0.07692,0.07354,0.06994,0.06595,0]
fpr, tpr, th = roc_curve(compare_data.apply_y, compare_data.scores3.values/100)
ks3 = tpr - fpr
print('all ks:   ' + str(max(ks3)))
print(roc_auc_score(compare_data.apply_y,compare_data.scores3.values/100))
r_p_chart2(compare_data.apply_y, compare_data.scores3.values/100, min_scores3, part=20)
