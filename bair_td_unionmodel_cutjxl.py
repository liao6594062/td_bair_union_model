# -*- coding: utf-8 -*-
"""
Created on Thu Aug 15 19:13:57 2019
...
vars_count_table=pd.read_excel('D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/de_dict_vars_20190808.xlsx') #手动增加了同盾外部评分
...
#    cat_less20 = ["deci_apt_facetrial_housetype","deci_cont02_relationship","deci_jxl_contact2_rel","deci_vehicle_minput_lastmortgagerinfo"]
    ...
    print(less_iv_name)
    less_iv_name.to_excel("D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/cat_less_iv_1.xlsx")
    df = v2_equif_bin(df, cat_over20, target)
    cols_name = [i+"_Bin" for i in cat_over20]
    over_iv_name = v2_cat_woe_iv(df, cols_name, target,0.02,"D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/")  # 计算woe 和IV值
    over_iv_name.to_excel("D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/cat_more_iv_2.xlsx")
...
df_choose1.to_excel('D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/df_choose1.xlsx',index=False)
target ="apply_y"
max_interval = 5
    ...
    for col in num_var:
        ...
        if -99 not in set(df[col]):
            ...
            if len(cutOff)!=0:
               ...
               regroup.to_csv("D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/regroup_Chi_cutbin__.csv", mode="a", header=True)
               print(regroup)
               num_list.append(col)
...
df_choose1.to_excel("D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/cut_bin_train.xlsx")
...
df_hig_iv = v2_cat_woe_iv(df_choose1,cols_list,target,0.02,"D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/")
df_hig_iv.to_excel("D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/high_iv_chi.xlsx")
...
df_hig_iv = v2_cat_woe_iv(df_choose1,cols_list+str_columns,target,0.02,"D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/")
df_hig_iv.to_excel("D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/high_iv_chi.xlsx")
...
init_choose_name_desc.to_excel('D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/init_choose_name_desc_20190809.xlsx',index=False)
...
连续变量的相关性分析
"""
init_choose_name_desc=pd.read_excel('D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/init_choose_name_desc_20190809.xlsx')
init_choose_table=init_choose_name_desc.loc[init_choose_name_desc['是否选用'].isin(['是']),:]#手动去除一些不能用的变量
init_choose_table=init_choose_table[init_choose_table['数据源'].isin(['聚信利报告','聚信利'])==False]#去掉聚信立变量
numeric_columns = init_choose_table.loc[init_choose_table.var_type.isin(['\ufeff数据型','数据型', '数字型','数值型']), 'var_name'].values.tolist()
str_columns = init_choose_table.loc[init_choose_table.var_type.isin(['字符型', '\ufeff字符型','字符型 ' ]), 'var_name'].values.tolist()
date_columns = init_choose_table.loc[init_choose_table.var_type.isin(['\ufeff日期型','日期型']), 'var_name'].values.tolist()
print(len(numeric_columns)+len(str_columns)+len(date_columns))
...
error_cols,df_choose3=main(df_choose2,col,'D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/')
...
df_high_cor1.to_excel("D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/df_high_cor.xlsx")


## 相关性去除iv更低的变量
cat_less_iv=pd.read_excel('D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/cat_less_iv_1.xlsx')
cat_less_iv=cat_less_iv.set_index('var_name').loc[str_columns,'IV'].reset_index().rename(columns={'IV':'iv'})
df_hig_iv=pd.read_excel("D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/high_iv_chi.xlsx")
df_hig_iv=pd.concat([df_hig_iv,cat_less_iv],axis=0)
df_hig_iv.to_csv('D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/df_hig_iv_all.csv',index=False)
df_high_cor1=pd.read_excel("D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/df_high_cor.xlsx")
...
main(df_choose5,remain_col,"D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归_剔除聚信立/final_test_num_bin_badrate1")
remain_col=['als_m12_id_nbank_mc_allnum_Bin',
...
main(df_choose6,remain_col,"D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归/final_test_num_bin_badrate2") 
...
vif_df.to_excel('D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归/vif_df_20190814.xlsx')
...
df_hig_iv = v2_cat_woe_iv(df_choose7,final_columns,'apply_y',0.02,"D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归/")
df_hig_iv=df_hig_iv.reset_index().rename(columns={'index':'var_name',0.0:'iv'}).sort_values('iv',ascending=False)


##最终剩下的变量
final_columns=['m_cnt_node_dist2_loan_all_all_Bin',
 'i_mean_freq_node_seq_partner_loan_all_all_Bin',
 'm_ratio_cnt_grp_max_partner_loan_all_all_Bin',
 'rational_score_Bin',
 'contact_unknown_contact_early_morning_Bin',
...
df_choose6['rational_score_Bin']=np.nan
df_choose6.loc[(df_choose1['rational_score']<=0)|(df_choose1['rational_score']>=601),'rational_score_Bin']='<=0&>=601'
df_choose6.loc[(df_choose1['rational_score']>0)&(df_choose1['rational_score']<492),'rational_score_Bin']='(0,492)'
df_choose6.loc[(df_choose1['rational_score']>=492)&(df_choose1['rational_score']<601),'rational_score_Bin']='[492,601)'
...
df_choose6.loc[df_choose1['jxl_id_comb_othertel_num']==1,'jxl_id_comb_othertel_num_Bin']='[1,2)'
df_choose6.loc[df_choose1['jxl_id_comb_othertel_num']>=2,'jxl_id_comb_othertel_num_Bin']='[2,inf)'
...
df_choose6['i_mean_freq_node_seq_partner_loan_all_all_Bin']=df_choose6['i_mean_freq_node_seq_partner_loan_all_all_Bin'].replace(
        {'b-1NA':'NA&[1.2,1.9375)','b1[1.2,1.4444)':'NA&[1.2,1.9375)','b2[1.4444,1.9375)':'NA&[1.2,1.9375)',
     'b0[-inf, 1.2)':'<1.2&>=1.9375','b4[1.9375,inf)':'<1.2&>=1.9375'}) 
df_choose6['scoreconson_Bin']=df_choose6['scoreconson_Bin'].replace({'b-1NA':'NA&>=651.0','b2[651.0,745.0)':'NA&>=651.0','b4[745.0,inf)':'NA&>=651.0'}) 
df_choose6['scoreafrevoloan_Bin']=df_choose6['scoreafrevoloan_Bin'].replace({'b-1NA':'NA&<44.0','b0[-inf, 44.0)':'NA&<44.0'
          ,'b2[81.0,84.0)':'b2[81.0,inf)','b4[84.0,inf)':'b2[81.0,inf)'}) 
...
df_choose6['contacts_class1_cnt_Bin']=df_choose6['contacts_class1_cnt_Bin'].replace({'b-1NA':'NA&b1[20.0,371.0)',
    'b1[20.0,371.0)':'NA&b1[20.0,371.0)','b2[371.0,461.0)':'b2[371.0,inf)','b4[461.0,inf)':'b2[371.0,inf)'})
...
main(df_choose6,final_columns,"D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归/final_test_num_bin_badrate4")  
df_hig_iv = v2_cat_woe_iv(df_choose6,final_columns,'apply_y',0.01,"D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归/")
...
choose_column.remove('m3_id_relate_homeaddress_num_Bin')
choose_column.remove('scoreafrevoloan_Bin')
choose_column.remove('max_total_amount_l6m_Bin')
choose_column.remove('contacts_class1_cnt_Bin')
...
#joblib.dump(lr,'D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归/lr_bair_tdunion_20190816.pkl')
...
#columns_coef.to_excel('D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归/bair_tdunion_columns_coef_20190816.xlsx',index=False)
...
outtime_testdata["scoreconson"+'_Bin'] = outtime_testdata["scoreconson"].map(lambda x: scoreconson(x))
outtime_testdata["scoreafrevoloan"+'_Bin'] = outtime_testdata["scoreafrevoloan"].map(lambda x: scoreafrevoloan(x))
...
for cn in choose_column:
    judge_stable_analyze(full_data,cn,'D:/llm/联合建模算法开发/百融加同盾联合建模_逻辑回归/调整分箱后的稳定性分析_加时间外验证/')
